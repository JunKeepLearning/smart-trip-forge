...
const DetailDrawer: React.FC = () => {
  ...
  const favorited = useMemo(() => {
    if (!item || !type || !favoritesList) return false;
    return favoritesList.some(fav => fav.item_id === item.id && fav.item_type === type);
  }, [favoritesList, item, type]);
  ...
  const handleFavoriteClick = (e: React.MouseEvent) => {
    ...
    if (favorited) {
      removeFavoriteMutation.mutate({ itemId: item.id, itemType: type });
    } else {
      addFavoriteMutation.mutate({ itemId: item.id, itemType: type });
    }
  };
  ...
  const renderDestinationContent = () => (
    <div className="space-y-6">
      <div className="aspect-video bg-muted rounded-lg overflow-hidden">
        <img src={(item as any).image} alt={(item as any).name} className="w-full h-full object-cover" />
      </div>
      <div>
        {renderTitle((item as any).name)}
        <p className="text-muted-foreground leading-relaxed">{(item as any).description}</p>
      </div>
      <div>
        <h3 className="text-lg font-semibold text-foreground mb-3">Popular Spots</h3>
        <div className="grid gap-3">
          {(item as any).highlights?.slice(0, 5).map((highlight: string, index: number) => (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-foreground font-medium">{(item as any).startCity} → {(item as any).endCity}</span>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <Calendar className="w-4 h-4 text-primary" />
          <span className="text-muted-foreground">Duration</span>
          <span className="text-foreground font-medium">{(item as any).days} days</span>
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <div className="space-y-4">
          {Array.from({ length: (item as any).days }, (_, i) => (
            <div key={i} className="flex gap-4">
              <div className="flex flex-col items-center">
                ...
                {i < (item as any).days - 1 && <div className="w-px h-8 bg-border mt-2" />}
              </div>
              <div className="flex-1 pb-4">
                <h4 className="font-medium text-foreground">Day {i + 1}</h4>
                <p className="text-sm text-muted-foreground mt-1">Explore {i === 0 ? (item as any).startCity : i === (item as any).days - 1 ? (item as any).endCity : 'amazing destinations'}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 text-muted-foreground mb-3">
          ...
          <span>{(item as any).destination}</span>
        </div>
        <p className="text-muted-foreground leading-relaxed">{(item as any).description}</p>
      </div>
      <div>
        <Badge variant="secondary" className="text-sm">{(item as any).category}</Badge>
      </div>
